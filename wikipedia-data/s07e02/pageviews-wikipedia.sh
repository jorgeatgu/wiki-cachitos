...
readarray -t artist < ~/github/wiki-cachitos/wikipedia-data/s07e01.txt
...
for (( i=0; i<${#artist[@]}; ++i )); do
    ...
    curl -s "GET" "https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/es.wikipedia/mobile-app/user/"${artist[$i]}"/daily/20190414/20190515" | jq '[.items[] | {"artista": .article, "fecha": .timestamp, "visitas": .views}]' > "${artist[$i]}"-"${type[$j]}".json
