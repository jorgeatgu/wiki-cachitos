...
for (( i=0; i<${#artist[@]}; ++i )); do
    ...
    curl -s "GET" "https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/es.wikipedia/mobile-app/user/${artist[$i]}/daily/20190318/20190619" | jq '[.items[] | {"artista": .article, "fecha": .timestamp, "visitas": .views}]' > "${artist[$i]}"-"${type[$j]}".json
